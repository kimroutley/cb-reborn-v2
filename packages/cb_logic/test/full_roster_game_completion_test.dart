  ...
  group(
    ...
    () {
      ...
      group('Comprehensive Game Flow Tests', () {
        test(
          'Scenario A: "The Full House" (Manual Setup, All Roles)',
          () async {
            ...
            print(
              'Scenario A: Full House finished. Winner: ${finalState.winner}',
            );
          },
        );

        test(
          'Scenario B: "Offensive Meta" (Auto-Assign, High Aggression)',
          () async {
            final container = ProviderContainer();
            addTearDown(container.dispose);
            final game = container.read(gameProvider.notifier);
            ...
            final state = container.read(gameProvider);
            final hasDealers = state.players.any(
              (p) => p.role.id == RoleIds.dealer,
            );
        ...
        test(
          'Scenario C: "Defensive Meta" (Auto-Assign, Protection Heavy)',
          () async {
            final container = ProviderContainer();
            addTearDown(container.dispose);
            final game = container.read(gameProvider.notifier);
      ...
      group('Targeted Mechanics Scenarios', () {
        test('Mechanic: Medic Save (Prevention of Death)', () async {
          ...
          final victimState = dayState.players.firstWhere(
            (p) => p.id == victim.id,
          );
          ...
          expect(
            dayState.lastNightReport.any((s) => s.contains('Medic protected')),
            isTrue,
          );
        });
        ...
        test('Mechanic: Whore Deflection (Sacrificial Lamb)', () async {
          ...
          final dealerState = endState.players.firstWhere(
            (p) => p.id == dealer.id,
          );
          final scapegoatState = endState.players.firstWhere(
            (p) => p.id == scapegoat.id,
          );
        ...
        test(
          'Mechanic: Clinger / Attack Dog (Partner Death Trigger)',
          () async {
            final container = ProviderContainer();
            final game = container.read(gameProvider.notifier);
            game.setGameStyle(GameStyle.manual);
            ...
            final dayState = container.read(gameProvider);
            final partnerState = dayState.players.firstWhere(
              (p) => p.id == partner.id,
            );
            final clingerState = dayState.players.firstWhere(
              (p) => p.id == clinger.id,
            );

            expect(
              partnerState.isAlive,
              isFalse,
              reason: "Partner should be dead",
            );
            expect(
              clingerState.isAlive,
              isFalse,

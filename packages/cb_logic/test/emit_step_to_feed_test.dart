...
void main() {
  ...
  group('Game.emitStepToFeed', () {
    ...
    test('emits narrative event when readAloudText is present', () {
      ...
      final narrativeEvent = updatedState.feedEvents.lastWhere(
        (e) => e.type == FeedEventType.narrative,
      );
    ...
    test('emits directive event when instructionText is present', () {
      ...
      final directiveEvent = updatedState.feedEvents.lastWhere(
        (e) => e.type == FeedEventType.directive,
      );
    ...
    test('emits action event when actionType is interactive', () {
      ...
      for (int i = 0; i < 20; i++) {
        ...
        if (_isInteractiveAction(step.actionType)) {
          ...
          final updatedState = container.read(gameProvider);
          final actionEvent = updatedState.feedEvents.lastWhere(
            (e) => e.type == FeedEventType.action,
          );
      ...
      () {
        ...
        for (int i = 0; i < 20; i++) {
          ...
          if (step.readAloudText.isNotEmpty &&
            ...
            final updatedState = container.read(gameProvider);
            final addedEvents = updatedState.feedEvents.sublist(
              initialEventsCount,
            );

            expect(
              addedEvents.any((e) => e.type == FeedEventType.narrative),
              true,
            );
            expect(
              addedEvents.any((e) => e.type == FeedEventType.directive),
              true,
            );
            expect(
              addedEvents.any((e) => e.type == FeedEventType.action),
              true,
            );
            break;
          }
    ...
    test(
      'timestamp and baseId are consistent across events from the same call',
      () {
        _addMockPlayers(game, 12);
        game.startGame();
    ...
    test(
      'emitCurrentStepNarrationVariationToFeed adds AI narrative event',
      () async {
    ...
    test(
      'emitCurrentStepNarrationVariationToFeed returns false when no step',
      () async {
        final success = await game.emitCurrentStepNarrationVariationToFeed();
